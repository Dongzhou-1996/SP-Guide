...
def _point_to_surface_pose(args: ExperimentArgs, map_name: str, pred_pos: Point2D) -> Pose4D:
    ...
    return Pose4D(pred_pos.x, pred_pos.y, z_around_center + 5, 0)
...
def goal_selection_gdino(
    ...
    for (map_name, obj_id, desc_id), pred_positions in tqdm(pred_goal_logs.items(), desc='selecting target bbox', unit='trajectory'):
        ...
        predicted_positions[(map_name, obj_id, desc_id)] = _point_to_surface_pose(args, map_name, pred_pos)
...
def goal_selection_gcf(
    ...
    predicted_positions = {}
    for eps_id, pred_goals in tqdm(pred_goal_logs.items(), desc='selecting target by GCF', unit='trajectory'):
        if not pred_goals:
            continue
        pred_progresses = pred_progress_logs.get(eps_id, [])
        gcf_goal = select_terminal_goal_from_logs(args, pred_goals, pred_progresses)
        if gcf_goal is None:
            continue
        predicted_positions[eps_id] = _point_to_surface_pose(args, eps_id[0], gcf_goal)
