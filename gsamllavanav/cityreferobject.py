...
from gsamllavanav.defaultpaths import OBJECTS_PATH, PROCESSED_DECRIPTIONS_PATH, get_data_root
...
def get_city_refer_objects(objects_path=None, processed_description_path=None) -> MultiMapObjects:
    ...
    if objects_path is None:
        objects_path = get_data_root() / "cityrefer" / "objects.json"
    if processed_description_path is None:
        processed_description_path = get_data_root() / "cityrefer" / "processed_descriptions.json"

    objects_path = Path(objects_path)
    processed_description_path = Path(processed_description_path)
...
def get_landmarks(objects_path=None, processed_description_path=None):
